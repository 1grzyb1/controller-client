...
class ControllerClient<T> {
  ...
  private MockHttpServletRequestBuilder prepareRequest(Method method, Object[] args) {
    ...
    setQueryParams(method, args, requestBuilder);
  ...
  private static void setQueryParams(
      Method method, Object[] args, MockHttpServletRequestBuilder requestBuilder) {
    var queryParams = getQueryParams(method, args);
    queryParams.forEach((key, value) -> requestBuilder.param(key, value.toString()));
  }
  ...
  private static Map<String, Object> getQueryParams(Method method, Object[] args) {
    var parameters = method.getParameters();
    return Arrays.stream(parameters)
